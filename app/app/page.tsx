...
export default function CoachTrollPage() {
  ...
  const [aboutToBlock, setAboutToBlock] = useState(false);
  ...
  const getAngerColor = () => {
    if (angerLevel < 60) return "bg-green-500";
    if (angerLevel < 120) return "bg-yellow-500";
    if (angerLevel < 160) return "bg-orange-500";
    return "bg-red-500";
  };

  const getCoachMood = () => {
    if (angerLevel < 40) return "cosmically aligned";
    if (angerLevel < 80) return "energy disturbed";
    if (angerLevel < 120) return "vibration disrupted";
    if (angerLevel < 160) return "universal flow blocked";
    return "cosmic chaos unleashed";
  };

  const getCoachEmoji = () => {
    if (angerLevel < 60) return "🧘‍♂️";
    if (angerLevel < 120) return "😤";
    if (angerLevel < 160) return "😠";
    return "🤬";
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
            ...
            if (line.startsWith("data: ")) {
              try {
                ...
                if (data.type === "metadata") {
                    ...
                    if (metadataString) {
                      try {
                        ...
                        if (metadata.blockUser === true) {
                          ...
                          setAboutToBlock(false); // Reset the flag
                          try {
                        ...
                        setAngerLevel(prev => Math.min(200, prev + 3));
                      }
                    } else {
                  ...
                  setAngerLevel(prev => Math.min(200, prev + 15));
                }
              } catch (parseError) {
      ...
      // Handle anger level updates from legacy metadata
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              <div className="w-24 sm:w-32 md:w-40">
                <Progress value={(angerLevel / 200) * 100} className={`h-2 sm:h-3 ${getAngerColor()}`} />
              </div>
          ...
          <div className="lg:col-span-3 order-1 lg:order-2">
            <Card className="h-[calc(100vh-120px)] sm:h-[800px] shadow-xl border-0">
              ...
              <CardContent className="p-0 h-[calc(100%-60px)] sm:h-[740px] flex flex-col">
                ...
                {/* Warning Message for About to Block */}
                {aboutToBlock && !sessionEnded && (
                  <div className="p-3 sm:p-4 border-t bg-red-50 border-red-200">
                    <div className="text-center">
                      <p className="text-red-600 font-bold text-sm animate-pulse">
                        🚨 CRITICAL WARNING: Your Life Coach is about to unleash ULTIMATE COSMIC RAGE!
                      </p>
                      <p className="text-red-500 text-xs mt-1">
                        Next disrespectful message will trigger the FINAL SPIRITUAL MELTDOWN before BLOCKING
                      </p>
                    </div>
                  </div>
                )}
